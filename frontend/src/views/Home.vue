  ...
  <el-container>
    <el-main>
      ...
      <el-row align="middle" type="flex">
        ...
        <el-col :xs="24" :lg="{span: 3, offset: 9}">
          <el-button class="big-button" :type="isGasStyle" plain style="border-radius: 14px; padding-top: 0px;">
            <h1>气体监测</h1>
            <p>{{ isGasShown }}</p>
          </el-button>
        </el-col>
        ...
        <el-col :xs="24" :lg="{span: 3, offset: 0}">
          <el-button class="big-button" :type="isFireStyle" plain style="border-radius: 14px; padding-top: 0px;">
            <h1>火焰监测</h1>
            <p>{{ isFireShown }}</p>
          </el-button>
        </el-col>
      ...
      <el-row align="middle" type="flex">
        <el-col :xs="24" :lg="{span: 3, offset: 9}">
          <el-button class="big-button" @click="windowFormVisible = true" :type="isRainAlert"  plain style="border-radius: 14px; padding-top: 0px;">
            <h1>门窗状态</h1>
            <p>点击查看</p>
          </el-button>
        </el-col>
    ...
    <el-dialog title="门窗状态" width="90%" :visible.sync="windowFormVisible">
      <el-table :data="windowData">
        <el-table-column property="name" label="名称" width="100"></el-table-column>
        <el-table-column property="status" label="状态" width="100"></el-table-column>
        <el-table-column property="rain" label="附加" width="100"></el-table-column>
      </el-table>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  computed: {
    isGasShown: function (){
      return this.isGas ? "异常" : "正常";
    },
    isFireShown: function (){
      return this.isFire ? "异常" : "正常";
    },
    isGasStyle: function (){
      return this.isGas ? "danger" : "";
    },
    isFireStyle: function (){
      return this.isFire ? "danger" : "";
    },
    isRainAlert: function (){
      return (this.isRain && !this.isWindow) ? 'danger' : '';
    }
  },
  methods: {
    queryData(){
      ...
      this.axios
          .get('http://47.104.141.250:9321/window/')
          .then(response => {
            this.isWindow = Boolean(Number(response.data['is_window']))
            this.isRain = Boolean(Number(response.data['is_rain']))
            if (!this.isWindow){
              this.windowData[0].status = '开启';
            } else {
              this.windowData[0].status = '关闭';
            }
            if (this.isRain){
              this.windowData[0].rain = '有雨滴';
            } else {
              this.windowData[0].rain = '无雨滴';
            }
          })
    },
